...
exports.run = async (client, message, args, params) => {
  ...
  if(!message.member.roles.has(`774967561405726720`) && !message.member.hasPermission('ADMINISTRATOR')) {
  let paradox = new Discord.RichEmbed()
  .setColor('RED')
  .setDescription(`Bu Komutu Kullana Bilmen İçin Warn Hammer Rolü Alman Lazım.
`) 
return message.channel.send(paradox)
}
  

  let cezalirol = "774866229604778015"; //Kullanıcı maksimum uyarı sayısını aştığında verilecek rol
